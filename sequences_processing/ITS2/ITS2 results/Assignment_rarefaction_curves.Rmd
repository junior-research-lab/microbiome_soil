...
As we can see on the graph and int the printed rowSums, B-9-3, B-7-3 and B-5-1 should be deleted, so that we can have a correct rarefied table
...
```
If we don't remove B-3-1, the number of read for each sample must be cutted to 3188. And that's not a lot. Maybe we should remove it also ??
